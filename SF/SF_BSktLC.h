...
private:
    ...
    int sk_lcsize = 100;
public:
    SF_BSktLC(int Mem, float ratio, float a, float b, int lcsize, int sk_depth){
        //Mem is bit;
        sf_alpha = a;
        sf_beta =b;
        sf_lcsize = lcsize;
        sk_lcsize = floor(lcsize/4);
        //sf_bktnum = floor((Mem * ratio)/(sizeof(BUCKET_node)*8));
        sf_bktnum = floor((Mem * ratio) / (SLOT_NUM*(sizeof(uint32_t)* 8*2 + sf_lcsize*2)));
        cout<<"number of bucket node: "<<sf_bktnum<<endl;
      //  SF = (BUCKET_node *)malloc(sizeof(BUCKET_node *));
